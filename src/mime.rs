pub use mime::{Mime, APPLICATION_OCTET_STREAM};
...
pub fn from_path<P: AsRef<Path>>(path: P) -> Option<String> {
    if let Some(ext) = path.as_ref().extension().and_then(OsStr::to_str) {
       if let Some(m) = get_mime_type(&ext) {
          if let Some(mime_str) = m.get(0) {
...
pub fn first_or_octet_stream(ext: String) -> String {
    if let Some(m) = get_mime_type(&ext) {
       if let Some(mime_str) = m.get(0) {
          return String::from(*mime_str)
       }
    }
    String::from(MIME_DEFAULT)
}
pub fn get_mime_type<'a>(ext: &str) -> Option<Vec<&'a str>> {
  MIME_TYPES.iter().
      find(|(k,_)| k == &ext).
      map(|(_,v)| v.to_vec())
}
